...
def Line2array(line,Select=False): 
	ll = line.split()
	if 'tv' in line.split()[0].lower(): 
		return ['Tv']+line.split()[-3:] 
	elif len(line.split())==4: 
		if Select == True: 
			A = input('All Ions is writting as "T T T " , Yes or No?:') 
			if A.lower().strip()[0] == 'y': 				 
				Ion = remFragm(ll[0]) 
				return [Ion,'0',ll[1],ll[2],ll[3]] 
			else: 
				print('Script is exiting, provide correct format of *.com file') 
				exit() 
		else:
			Ion=remFragm(ll[0]) 
			return [Ion,ll[1],ll[2],ll[3]]
		...
		if line.split()[1].lstrip('-+').isdigit():
			...
			if 'fragment' in ll[1].lower(): 
				A = input('All Ions are writen as "T T T " , Yes or No?:') 
				if A.lower().strip()[0] == 'y': 
					Ion = remFragm(ll[0]) 
					return [Ion,'0',ll[1],ll[2],ll[3]] 
				else: 
			...
			while line: 
				if not len(line.strip()) < 1: 
					if not 'tv' in line.lower().split()[0]: 
						#print(line)
						Coord.append( Line2array(line,Select)) 
						print(Line2array(line,Select))
						try: 
							line = file.readline() 
						except: 
							break 
						...
						#
						TV.append(Line2array(line,Select))  
						try: 
							line = file.readline() 
						except: 
							break 
